...
def plot_feature_importance_bar_chart(feature_importance, figure_name):
...
def plot_real_predicted_scatter(preds_and_feats, figure_name):
    ...
    preds_and_feats = preds_and_feats.rename(columns = PRETTY_NAMES)
    ...
    for feat in preds_and_feats.columns:
        ...
        plt.savefig('reports/figures/' + figure_name + '_' + feat.replace(' ', '').replace('\n', '').replace('*', '') + '_scatter.png')
        plt.show()
...
def plot_correlation_coefficient(features_w_anc, figure_name):
    sns.set(font_scale = .7, style = 'whitegrid')
    plt.figure(figsize = (6,5))
    ...
    feat_corr_df = pd.DataFrame(columns = ['Feature', corr_col])
    features_w_anc = features_w_anc.rename(columns = PRETTY_NAMES)
    
    corr_data = features_w_anc.corr().loc[case_col]
    pvalue_data = calculate_pvalues(features_w_anc).loc[case_col]

    feat_corr_df['Feature'] = corr_data.index
    feat_corr_df[corr_col] = corr_data.tolist()
    feat_corr_df['p-value'] = pvalue_data.tolist()
    ...
    plt.savefig('reports/figures/' + figure_name + '.png', bbox_inches = 'tight')
    return feat_corr_df
